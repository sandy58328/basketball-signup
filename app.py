...
def delete_player(pid, d):
    ...
    if target:
        tn = target['name']
        if "(友" in tn: current_data["sessions"][d] = [p for p in current_data["sessions"][d] if p['id'] != pid]
        else: current_data["sessions"][d] = [p for p in current_data["sessions"][d] if p['id'] != pid and not p['name'].startswith(f"{tn} (友")]
    ...
    for idx, p in enumerate(lst):
        ...
        if st.session_state.edit_target == p['id']:
            with st.form(key=f"e_{p['id']}"):
                en = st.text_input("姓名", p['name'], disabled=True)
