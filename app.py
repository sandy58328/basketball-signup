    ...
    for idx, p in enumerate(lst):
        ...
        if st.session_state.get('edit_target') == p['id']:
            with st.form(key=f"e_{p['id']}"):
                ...
                if st.form_submit_button("💾 儲存", type="primary"): update_player(p['id'], dk, en, p.get('isMember'), eb, ec, ev)
...
with st.expander("⚙️ 管理員登入 (Admin Login)", expanded=st.session_state.is_admin):
    if not st.session_state.is_admin:
        ...
        if cur_all_s:
            del_s = st.selectbox("刪除場次", cur_all_s)
            if st.button("🗑️ 確定刪除"):
                cur = load_data(); del cur["sessions"][del_s]; save_data(cur); st.rerun()
            h_s = st.multiselect("隱藏場次", cur_all_s, default=st.session_state.data.get("hidden", []))
            if st.button("💾 更新隱藏"):
        ...
        # 這裡改用最直接的邏輯：把 leaves 字典轉成清單直接顯示
        l_data = st.session_state.data.get("leaves", {})
        
        # 建立一個包含所有請假項目的清單，方便刪除
        leave_records = []
        for name, months in l_data.items():
            for m in months:
                leave_records.append({"name": name, "month": m})
        
        if leave_records:
            st.info(f"系統目前共有 {len(leave_records)} 筆請假資料")
            for rec in leave_records:
                c1, c2 = st.columns([3, 1])
                c1.markdown(f"**{rec['name']}**：{rec['month']}")
                if c2.button("刪除假單", key=f"del_{rec['name']}_{rec['month']}"):
                    cur = load_data()
                    if rec['name'] in cur["leaves"]:
                        cur["leaves"][rec['name']].remove(rec['month'])
                        if not cur["leaves"][rec['name']]: del cur["leaves"][rec['name']]
                        save_data(cur)
                        st.rerun()
        else:
            st.warning("⚠️ 警告：後台抓不到任何請假資料。請確認上方「休假公報」是否有顯示人名。")
        ...
        if st.button("📊 產生出席報表"):
            ...
            except Exception: st.error("統計失敗")
