...
def delete_player(pid, dk):
    """刪除報名球員"""
    cur = load_data(); target = next((x for x in cur["sessions"][dk] if x['id'] == pid), None)
    if target:
        tn = target['name']
        if "(友" in tn: cur["sessions"][dk] = [x for x in cur["sessions"][dk] if x['id'] != pid]
        else: cur["sessions"][dk] = [x for x in cur["sessions"][dk] if x['id'] != pid and not x['name'].startswith(f"{tn} (友")]
        save_data(cur); st.rerun()

def promote_player(wid, dk):
    """遞補功能"""
    ...
    t_m = next((x for x in cur["sessions"][dk] if x['id']==wid), None)
...
def render_list_items(lst, dk, is_wait=False, can_edit=True, is_adm=False):
    """渲染報名名單 (徹底解決 NameError)"""
    if not lst:
        ...
        for k, v in ld.items():
            if v: 
                ...
                for month in sorted(v):
                    # 【核心修復】大廳名字旁邊的垃圾桶，只有管理員看得到
                    col_txt, col_btn = st.columns([3, 1.2])
                    col_txt.markdown(f"👤 **{k}**: {month}")
                    if st.session_state.is_admin:
                        if col_btn.button("🗑️", key=f"hall_del_{k}_{month}"):
...
if not dates: st.info("👋 目前沒有開放報名的場次")
    ...
    for i, dk in enumerate(dates):
        with tabs[i]:
            ...
            st.subheader("🏀 報名名單")
            render_list_items(main, dk, False, can_e, st.session_state.is_admin)
            if wait:
                st.markdown("<br>", unsafe_allow_html=True); st.subheader("⏳ 候補名單"); render_list_items(wait, dk, True, can_e, st.session_state.is_admin)
    ...
    if not st.session_state.is_admin:
        ...
        if recs:
            ...
            for r in recs:
                ...
                col1.markdown(f"👤 **{r['name']}** ({r['month']})")
                if col2.button("刪除", key=f"adm_del_leave_{r['name']}_{r['month']}"):
                    delete_leave_record(r['name'], r['month'])
        else: st.info("目前無人請假")
