        ...
        if st.form_submit_button("送出假單"):
            if l_name:
                ...
                if leave_str not in current_data["leaves"][l_name]:
                    ...
                    time.sleep(1)
                    st.rerun()
                else: st.warning(f"{l_name} 已經登記過 {leave_str} 的假囉！")
            else: st.error("請輸入姓名")

# 場次顯示
all_dates = sorted(st.session_state.data["sessions"].keys())
hidden = st.session_state.data.get("hidden", [])
dates = [d for d in all_dates if d not in hidden] # 預設只顯示公開場次
        ...
        with tabs[i]:
            ...
            def delete(pid, d):
                ...
                if target:
                    ...
                    if "(友" in target_name: current_data["sessions"][d] = [p for p in current_data["sessions"][d] if p['id'] != pid]
                    else: current_data["sessions"][d] = [p for p in current_data["sessions"][d] if p['id'] != pid and not p['name'].startswith(f"{target_name} (友")]
                    if st.session_state.edit_target == pid: st.session_state.edit_target = None
                    save_data(current_data)
                    st.toast("🗑️ 已刪除"); time.sleep(0.5); st.rerun()

            with st.expander("📝 點擊報名 / 規則說明", expanded=not is_locked):
                if is_locked: st.warning("⛔ 已截止")
                        ...
                        if name:
                            ...
                            if error_message: st.error(error_message)
                                ...
                                for k in range(tot):
                                    ...
                                    if is_main: final_name = name; p_im, p_bb, p_oc = im, bb, oc; p_count = 0 if ev else 1
                                    else:
                                        ...
                                        final_name = f"{name} (友{friend_seq})"; p_im, p_bb, p_oc = False, False, False; p_count = 1 
                                ...
                                st.balloons(); st.toast(f"🎉 歡迎 {name} 加入！", icon="🏀"); time.sleep(1.5); st.rerun()
                        else: st.toast("❌ 請輸入姓名")
            ...
            def render_list(lst, is_wait=False):
                ...
                for idx, p in enumerate(lst):
                    if p.get('count', 1) > 0: display_counter += 1; index_str = f"{display_counter}."; idx_class = "list-index"
                    else: index_str = "🌸"; idx_class = "list-index-flower"
                    if st.session_state.edit_target == p['id']:
                        ...
                        c_cfg = [7.8, 0.6, 0.6, 1.0] 
                        cols = st.columns(c_cfg, gap="small")
                        with cols[0]: st.markdown(f"""<div class="player-row"><span class="{idx_class}">{index_str}</span><span class="list-name">{p['name']}</span>{badges}</div>""", unsafe_allow_html=True)
                        if can_edit:
                            b_idx = 1
                            if b_idx < len(cols):
                                is_friend = "(友" in p['name']
                                if not is_friend:
                                    with cols[b_idx]: st.markdown('<div class="list-btn-col list-btn-e">', unsafe_allow_html=True); st.button("✏️", key=f"be_{p['id']}", on_click=lambda id=p['id']: setattr(st.session_state, 'edit_target', id)); st.markdown('</div>', unsafe_allow_html=True)
                            if b_idx+1 < len(cols):
                                with cols[b_idx+1]: st.markdown('<div class="list-btn-col list-btn-d">', unsafe_allow_html=True); st.button("❌", key=f"bd_{p['id']}", on_click=delete, args=(p['id'], date_key)); st.markdown('</div>', unsafe_allow_html=True)
            render_list(main)
            if wait:
