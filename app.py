...
def delete_player(pid, d):
    ...
    if target:
        ...
        if "(友" in tn:
            ...
            current_data["sessions"][d] = [p for p in current_data["sessions"][d] if p['id'] != pid and not p['name'].startswith(f"{tn} (友")]
        st.session_state.edit_target = None
        save_data(current_data); st.toast("🗑️ 已刪除"); time.sleep(0.5); st.rerun()

def promote_player(wid, d):
    current_data = load_data()
    pl = sorted(current_data["sessions"][d], key=lambda x: x.get('timestamp', 0))
    main_p, curr = [], 0
    for p in pl:
    ...
    tg = next((p for p in reversed(main_p) if not p.get('isMember')), None) 
    if w and tg:
       tg_ref = next((p for p in current_data["sessions"][d] if p['id']==tg['id']), None)
       w['timestamp'], tg_ref['timestamp'] = tg_ref['timestamp'] - 1.0, main_p[-1]['timestamp'] + 1.0
    ...
    for idx, p in enumerate(lst):
        ...
        if st.session_state.edit_target == p['id']:
            with st.form(key=f"e_{p['id']}"):
                en = st.text_input("球員姓名 (不可改)", p['name'], disabled=True)
                ...
                b1, b2 = st.columns(2)
                if b1.form_submit_button("💾 儲存", type="primary"): update_player(p['id'], dk, en, p.get('isMember'), eb, ec, ev)
            ...
            # 手機版自適應欄位
            cols = st.columns([7.8, 0.6, 0.6, 1.0] if not (is_adm and is_wait) else [6.5, 1.2, 0.6, 0.6, 1.1], gap="small")
            ...
            if is_adm and is_wait and p.get('isMember'):
                if cols[b_idx].button("⬆️", key=f"up_{p['id']}"): promote_player(p['id'], dk)
                b_idx += 1
            if can_edit:
                if b_idx < len(cols) and "(友" not in p['name']:
                    if cols[b_idx].button("✏️", key=f"be_{p['id']}"): st.session_state.edit_target = p['id']; st.rerun()
                if b_idx+1 < len(cols):
                    if cols[b_idx+1].button("❌", key=f"bd_{p['id']}"): delete_player(p['id'], dk)
...
with c_l1:
    with st.expander("🏖️ 我要請假 (長假登記)"):
        with st.form("hall_leave_form", clear_on_submit=True):
    ...
    for i, dk in enumerate(dates):
        with tabs[i]:
            ...
            if wait_p:
                ...
                render_list(wait_p, dk, True, can_e, st.session_state.is_admin)
...
with st.expander("⚙️ 管理員登入 (Admin Login)", expanded=st.session_state.is_admin):
    if not st.session_state.is_admin:
        ...
        nd = st.date_input("新增報名場次", min_value=date.today())
        if st.button("➕ 確認新增場次"):
            cur = load_data()
            if str(nd) not in cur["sessions"]: 
                cur["sessions"][str(nd)] = []; save_data(cur); st.rerun()
        
        cur_all_s = sorted(st.session_state.data["sessions"].keys())
        if cur_all_s:
            del_s = st.selectbox("選擇要刪除的場次", cur_all_s)
            if st.button("🗑️ 確定刪除此日期"):
                cur = load_data(); del cur["sessions"][del_s]; save_data(cur); st.rerun()
            
            h_s = st.multiselect("隱藏不公開場次", cur_all_s, default=st.session_state.data.get("hidden", []))
            if st.button("💾 更新隱藏設定"):
        ...
        has_l = False
        for lname, lmonths in l_data.items():
            if lmonths:
                has_l = True
                st.markdown(f"**👤 {lname}**：{', '.join(lmonths)}")
                # 每個有請假的人，都有一個下拉選單和刪除按鈕
                dm = st.selectbox(f"要刪除 {lname} 的假？", ["請選擇"] + lmonths, key=f"adm_l_{lname}")
                if dm != "請選擇":
                    if st.button(f"🗑️ 確定刪除 {lname} {dm} 的假單", key=f"btn_l_{lname}"):
                        cur = load_data()
                        if lname in cur["leaves"] and dm in cur["leaves"][lname]:
                            cur["leaves"][lname].remove(dm)
                            if not cur["leaves"][lname]: del cur["leaves"][lname]
                            save_data(cur); st.rerun()
        if not has_l: st.info("目前系統內無人請假")

        # 3. 出席統計
        st.divider()
        st.subheader("3. 出席統計與警告")
        if st.button("📊 產生出席報表"):
            try:
                ...
                for ds, pl in d_m["sessions"].items():
                    ...
                    if d_obj <= today_obj:
                        for p in pl:
                            if "(友" not in p['name']:
                                if p['name'] not in ls or d_obj > ls[p['name']]: ls[p['name']] = d_obj
                rep = []
                for n, do in ls.items():
                    df = (today_obj - do).days
                    onl = any(m in d_m["leaves"].get(n, []) for m in [today_obj.strftime("%Y-%m")])
                    stt = "🏖️ 請假" if onl else "🔴 踢出" if df > 60 else "🟢 活躍"
                    rep.append({"姓名": n, "最後出席": str(do), "未出席天數": df, "狀態": stt})
                st.dataframe(rep, hide_index=True)
            except: st.error("統計資料計算失敗")
