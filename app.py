...
st.markdown("""
    ...
    /* [V3.48 Upgrade] 核彈級隱藏所有系統標記 */
    /* 1. 隱藏最上方的系統 Header (包含漢堡選單、彩條、連線狀態) */
    header {
        visibility: hidden !important;
        height: 0px !important;
    }
    
    /* 2. 針對特定 ID 與 Class 強制移除 */
    #MainMenu {display: none !important;} /* 右上角選單 */
    footer {display: none !important;}    /* 下方 Made with Streamlit */
    
    /* 3. 隱藏右上角的系統工具列 (Deploy, Record, Settings...) */
    ...
    for i, date_key in enumerate(dates):
        with tabs[i]:
            ...
            if wait:
                ...
                st.subheader(f"⏳ 候補名單")
                render_list(wait, is_wait=True)mport streamlit as st
import streamlit.components.v1 as components
import json
import os
import time
