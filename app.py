...
with st.expander("⚙️ 管理員專區 (Admin)", expanded=st.session_state.is_admin):
    if not st.session_state.is_admin:
        ...
        st.markdown('</div>', unsafe_allow_html=True)
        render_stats(st.session_state.data)# ── 請假 / 休假公報 ──
